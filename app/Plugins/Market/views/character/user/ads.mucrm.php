<div class="bg-zinc-950 text-zinc-400">
    @view('components.web.title', ['title'=> __lang('plugin.market.character.my_ads')])

    <div class="flex justify-center my-6">
        <a
            href="{{ route('plugins.market.character.index') }}"
        ...
        @forelse($characters as $character)
            <div x-data="{ open: false }" class="lg:col-span-12 grid grid-cols-1 lg:grid-cols-12 gap-6 border-b border-zinc-900 pb-6 last:border-0">
                ...
                <div class="lg:col-span-8 space-y-6">
                    ...
                    <form action="{{ route('plugins.market.character.delete', ['id' => $character->id]) }}" method="POST" class="bg-zinc-900 border border-zinc-800 rounded-lg p-5 space-y-4">
                        @csrf
                        @method('DELETE')
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 items-end">
                            ...
                            >
                                {{ __lang('plugin.market.character.removed') }}
                            </button>
                        </div>
                    </form>
                </div>
